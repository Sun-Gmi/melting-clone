...
/** 탐색 화면 카드. 누르면 바로 대화방이 아니라 캐릭터 소개 팝업이 먼저 뜬다. */
export default function CharacterCard({ c, rank }: { c: CharacterProfile; rank?: number }) {
  ...
  return (
    <>
      <button
        ...
        aria-label={`${c.name} 소개 보기`}
        className="group block w-full text-left transition-transform duration-200 hover:scale-[1.02]"
      >
      ...
      {open && <CharacterProfileModal c={c} onClose={() => setOpen(false)} />}
    </>
  );
}
