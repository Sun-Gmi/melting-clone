...
app/
  ...
  api/chat/route.ts        ★ 채팅 API. GET 불러오기 / POST 답변+호감도 갱신 / DELETE 대화 초기화
...
lib/
  ...
  prompt.ts                ★ 페르소나 + 현재 호감도 → LLM 시스템 프롬프트. 대화 품질의 핵심
  affinity.ts              호감도 규칙 (0~100, 턴당 ±5, 5단계) + 답변 끝 [호감도 +N] 태그 파서
...
7. **호감도는 LLM 이 판정, 서버가 적용** — 답변 마지막 줄의 `[호감도 +N]` 태그를 서버가 떼어내
   `conversations.affinity` 에 반영하고(0~100, 턴당 ±5), 현재 값과 단계(차가움/낯섦/호기심/설렘/애착)를
   다음 턴 프롬프트에 넣어 말투가 달라지게 한다. JSON 모드 대신 태그를 쓴 이유: 모델이 잊어도 답변은 멀쩡하다.

8. **LLM 은 Gemini 무료 티어** — 사용자가 "가장 저렴한 것 / 무료" 를 원했습니다.
...
| **호감도 판정 품질** | LLM 이 답변 끝에 `[호감도 +N]` 을 적는 방식. 태그를 빠뜨리면 변화 0 으로 처리(답변은 정상). 판정이 후하거나 짜면 `lib/prompt.ts` 의 [호감도 판정] 기준을 손볼 것 |
...
1. **스트리밍 응답** — 한 글자씩 흘려보내 체감 속도 개선 (Gemini `streamGenerateContent`)
2. **탐색 페이지 필터 동작** — 태그 클릭 시 실제 필터링
3. **캐릭터 만들기** — "창작" 탭

완료: ~~대화 목록 화면~~ · ~~별표 지문 표시~~ · ~~캐릭터 소개 팝업~~ · ~~호감도 실제 동작~~ · ~~대화 다시 시작~~ (모두 2026-09-17)
